...
if ($_POST['type1'] == 9) $_POST['name1']=27;
...
/*
echo "save_parameters_pic_com_id_tab -- <pre>";
print_r($_POST['save_parameters_pic_com_id_tab']);
echo "</pre>";

echo "save_parameters_pic_com_val_tab -- <pre>";
print_r($_POST['save_parameters_pic_com_val_tab']);
echo "</pre>";
*/
...
if ($_POST['save_parameters_pic_com1_id_tab'] != '') {
	foreach ($_POST['save_parameters_pic_com1_id_tab'] as $key => $value) {

		$value1=explode('_',$value);
		$questionid=$value1[2];
		$elementid1=$value1[3];		
		$pic_com1_tab[$questionid][$elementid1]=$_POST['save_parameters_pic_com1_val_tab'][$key];
		//echo $questionid.' - '.$elementid1." - ".$_POST['save_parameters_pic_com1_val_tab'][$key].'<br>';
	}
}
/*
echo "pic_com1_tab -- <pre>";
print_r($pic_com1_tab);
echo "</pre>";
*/
...
foreach ($_POST['save_parameters_id_tab'] as $key => $value) {
	...
	if ($questions1_tab[$questionid] == 1) {
		...
		if ($save_parameters_val_tab1 != '') {
			foreach ($save_parameters_val_tab1 as $key2 => $value2) {
				$answerid1 .= $value2.':'.$pic_com_tab[$questionid][$value2].',';
			}
		...
		foreach ($managers_tab as $key2 => $value2) {
			$answerid1 .= $key2.':'.$pic_com_tab[$questionid][$key2].',';
		}
		$answerid1=substr($answerid1,0,-1);		
		...
		if ($_POST['website_width'] > 650) {
			
			foreach ($managers_tab as $key2 => $value2) {
				$answer .= $value2.':'.$pic_com1_tab[$questionid][$key2].',';
			}
			$answer=substr($answer,0,-1);		

			foreach ($managers_tab as $key2 => $value2) {
				$answerid1 .= $key2.':'.$pic_com1_tab[$questionid][$key2].',';
				$_POST['save_parameters_val_tab'][$key] .= $key2.',';
			}
			$answerid1=substr($answerid1,0,-1);		
			$_POST['save_parameters_val_tab'][$key]=substr($_POST['save_parameters_val_tab'][$key],0,-1);			
	...
	$questiones_check1_tab[$questionid]++;

	$insert=1;
	/*
	if ($_POST['website_width'] > 650 AND $questions1_tab[$questionid] == 10 AND $questiones_check1_tab[$questionid] == 1) {
		$insert=0;
	} elseif ($_POST['website_width'] <= 650 AND $questions1_tab[$questionid] == 10 AND $questiones_check1_tab[$questionid] == 2) {
		$insert=0;
	}
	*/
	...
	if ($insert == 1) {
		...
		'".mysql_real_escape_string($answerid1)."',
		...
		//echo "website_width -- ".$_POST['website_width']." <br>";
		//echo "questionid -- ".$questiones_check1_tab[$questionid]." <br>";
}	
