...
//include_once("results_slt_login.php");
...
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {
	...
	foreach ($row as $key => $value) {
		$users_tab[$row['userid']][$key]=$value;
	}
}

$info_list0_tab[1]='Elavon SLT \'Leadership & Team Effectiveness\' survey';
$info_list0_tab[2]='Elavon SLT \'Leadership & Team Effectiveness\' survey';

$info_list1_tab[1][1]='How many Top 100 people know your name?';
...
$query = "SELECT DISTINCT questionnaries.questionnarieid,type  FROM questionnaries,questionnaries_answers WHERE questionnaries.questionnarieid=questionnaries_answers.questionnarieid AND questionnaries.questionnarieid>=266 
	...
	if ($row['user'] == 27 OR $test1_tab[$row['user']] == $row['questionnarieid']) {
		...
		if ($row['type'] == 9 AND $row['questionid'] == 135) {
			...
			foreach ($answerid1_tab as $key => $value) {
				...
				foreach ($users_tab as $key1 => $value1) {

					//echo $answerid2_tab[0]." == ".$key1."<br>";
					if ($answerid2_tab[0] == $key1) {

						$answerid3_tab=explode(' ',strtolower($users_tab[$key1]['name']));
		...
		if ($row['answers_groupid'] == 4) {	
			...
			//if (strstr($row['answerid'], ','.$selected_userid.',')) {
				...
				foreach ($answerid1_tab as $key0 => $value0) {	

					$answerid2_tab=explode(',',$value0);	
...
// questions_groups
$query = "SELECT * FROM questions_groups WHERE usersgroupid=9 ORDER BY position ASC";		
$result = @mysql_query ($query);
$questions_groups_tab1='';
$questions_groups_tab2='';
$i=0;
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {

	$i++;
	$questions_groups_tab1[$row["questions_groupid"]]=$row['name'];
	$questions_groups_tab2[$row["questions_groupid"]]=$i;
}

/*
echo "questions_groups_tab1 -- <pre>";
print_r($questions_groups_tab1);
echo "</pre>"; 

echo "questions_groups_tab2 -- <pre>";
print_r($questions_groups_tab2);
echo "</pre>"; 
*/

//get data in order like in survey
$query = "SELECT 
questionid,
name,
charter,
...
answers_groupid
FROM 
questions WHERE questionid>0 AND usersgroupid=9 AND questionid!=134 ORDER BY position ASC";		
$result = @mysql_query ($query);
$questions_tab1='';
while ($row = mysql_fetch_array ($result, MYSQL_ASSOC)) {

	$questions_tab1[$row["questions_groupid"]][$row["questionid"]]=$row['name'];
}

/*
echo "questions_tab1 -- <pre>";
print_r($questions_tab1);
echo "</pre>"; 
...
include_once('results_top100_disp.php');
